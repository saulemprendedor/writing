...
The second piece came from a Salesforce presentation about AI features inside Slack. That's where the idea hit me: **if the whole team already lives in Slack, why are the tools of the job not there?**
...
The challenge wasn't technical, it was one of standard. The tools I put in that thread had to be **as good as or better than** the ones each area already used. If they were worse, nobody would move — and rightly so.

So I took the experiment to Slack, and the implementation that ran on my machine moved to ephemeral sandboxes in the cloud. And something showed up that I hadn't gone looking for: I could work on several features **at once**.

## The architecture decision: the agent doesn't drive
...
The rule I took from it: **orchestrate the process explicitly, delegate the loop.** The process is yours because it's your engineering judgment. The implement-and-fix-itself loop isn't, and competing with it's misplaced pride.

That set of principles is what I ended up calling **DEV Genius**. It isn't a tool: it's the way the line of work is built around the model.

One uncomfortable question is left, and it's the one for the next article: that agent runs **without asking for permissions**. How do you do that without it being reckless?

---

*Second article in a series of five.*

*Continues in chapter 3: [An agent that runs without asking for permissions: how to make it secure](https://saul.botsmith.ai/en/blog/correr-sin-permisos).*
